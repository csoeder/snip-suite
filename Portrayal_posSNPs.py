	...
	for chro in chroms:						#For each chromosome...
		...
		for i in inter:						#for each shared site...
			if p1[chro][i] != p2[chro][i]:	#if the phenotype at the site is actually different...
				inter.pop(index(i))			#remove it from the shared SNPS
...
present1, present2, n00bs = snp_grep(disjoint1_SNPs, disjoint2_SNPs, hybrid_SNPS_file)

pickle.dump( [shared_SNPs, disjoint1_SNPs, disjoint2_SNPs, present1, present2, n00bs], open('%s.pickle'%titleHyb, 'wb') )
